...
`(memo-fib 3)` saves one step by not having to recompute `(fib 1)`.

Ignoring table lookups, `(memo-fib n)` takes a number of steps proportional to $n$ because each step is computed exactly once.

`(memoize fib)` does not work as expected. Only the result is memoized, not the recursive calls.
