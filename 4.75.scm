...
;; Test
(load "4.75.logic.microshaft.run.scm")

(unique (job ?x (computer wizard)))

(assert! (rule (key-person ?person ?job)
               (and (job ?person (?department . ?job))
                    (unique (job ?anyone (?department . ?job))))))
(key-person ?who ?department)

(assert! (rule (has-single-report ?manager ?employee)
               (and (supervisor ?employee ?manager)
                    (unique (supervisor ?anyone ?manager)))))
(has-single-report ?who ?employee)
