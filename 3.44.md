...
Unlike exchanges, transfers do not depend on account state. As long as `withdraw` and `deposit` are serialized within each account, transfers will execute concurrently without issue.
